...
const ChangePassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        // Update token if new one is provided (contains updated isTempPassword flag)
        if (response.token) {
      ...
      >
        <div className="bg-white rounded-[40px] shadow-2xl overflow-hidden p-10">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 mt-2">You must change your temporary password to continue</p>
          </div>

          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl mb-6 flex gap-3">
            <AlertCircle className="text-amber-600 shrink-0" size={20} />
            <p className="text-sm text-amber-800 font-medium">
              This is a mandatory security requirement. You cannot access other features until you change your password.
            </p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 text-center">
            <p className="text-xs text-slate-400 font-medium">
              Having trouble? Contact your warden for assistance.
            </p>
          </div>
        </div>
      </motion.div>
